...
import com.burgerplace.main.boardPage.entity.QTradeBoard;
import com.burgerplace.main.boardPage.entity.QTradeReply;
...
public class TradeBoardSearchImpl extends QuerydslRepositorySupport implements TradeBoardSearch {
    ...
    public Page<TradeBoard> search1(String searchType, String keyword, Pageable pageable) {
        ...
        QTradeBoard board = QTradeBoard.tradeBoard;

        JPQLQuery<TradeBoard> query = from(board);
        ...
        if (keyword != null && searchType != null) {
            ...
            for (String type : searchArr) {
                ...
                switch (type) {
                    case "t" -> searchBuilder.or(board.tradeTitle.contains(keyword));
                    case "c" -> searchBuilder.or(board.tradeContent.contains(keyword));
    ...
    public Page<Object[]> searchWithRcnt(String searchType, String keyword, Pageable pageable) {
        ...
        QTradeBoard board = QTradeBoard.tradeBoard;
        QTradeReply reply = QTradeReply.tradeReply;

        JPQLQuery<TradeBoard> query = from(board);
        // select * from FreeBoard

        query.leftJoin(reply).on(reply.tradeBoard.eq(board));
        ...
        JPQLQuery<Tuple> tupleQuery = query.select(board.tradeBno, board.tradeTitle, board.nickname,
                reply.countDistinct());
